    ...
    public class RiftCoroutine : IDisposable, ICoroutine
    {
        ...
        private bool IsRiftPortalOpen
        {
            get
            {
                ...
                {
                    return ZetaDia.Actors.GetActorsOfType<DiaGizmo>(true).Any(g => g.ActorSnoId == RiftData.RiftEntryPortalSNO);
                }
                else
                {
                    return ZetaDia.Actors.GetActorsOfType<DiaGizmo>(true).Any(g => g.ActorSnoId == RiftData.GreaterRiftEntryPortalSNO);
